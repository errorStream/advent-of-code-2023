        ...
        public long Run(StreamReader streamReader)
        {
            ...
            while (open.Count > 0)
            {
                ...
                foreach (Vec2 direction in Offsets)
                {
                    ...
                    {
                        if (curr.ConsecutiveLength == 10)
                        {
                            continue;
                        }
                        else if (curr.ConsecutiveLength > 10)
                        {
                    ...
                    {
                        const int minConsecutiveLength = 4;

                        if (curr.ConsecutiveLength < minConsecutiveLength && curr.Direction != new Vec2(0, 0))
                        {
                            continue;
                        }
                        ...
                        if (MaybeGetWeight(curr.Position + (direction * minConsecutiveLength)) is null)
                        {
                            continue;
                        }
